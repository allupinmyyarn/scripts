#!/usr/bin/perl -w
 ...
 ########################################################################
 # This script takes as input a list of ids (one id per line, but can   #
 # have more than 1 ID file) and reads a sequence file containing these #
 # sequences. It will return one file per id list with the sequences of #
 # that list (-o flag) Or print them all to STDOUT (default). If sure   #
 # that the names in id and FASTA files are identical (*sure*), use -i. #
 ########################################################################
